...
export function FlashcardView({
  ...
  if (!cardData) {
    return (
      <Card className='w-full max-w-lg h-80 sm:h-96 flex items-center justify-center shadow-xl bg-muted/50'>
        <CardContent>
          <p className='text-lg sm:text-xl text-muted-foreground font-headline'>
            No card to display for this level.
          </p>
        </CardContent>
      </Card>
    );
  ...
  const FrontContentInternal = () => (
    <div className='flex flex-col items-center justify-center h-full p-4 sm:p-6 text-center'>
      <h2 className='font-headline text-4xl sm:text-5xl mb-3 sm:mb-4' lang='ja'>
        {cardData.japanese}
      </h2>
      <p className='text-muted-foreground text-md sm:text-lg'>
        ({cardData.partOfSpeech} - {cardData.jlptLevel})
      </p>
      <Button
        variant='ghost'
        size='icon'
        onClick={(e) => {
          e.stopPropagation();
          onPronounce(cardData.japanese, 'ja-JP');
        }}
    ...
    <div className='flex flex-col h-full p-3 sm:p-4 space-y-3'>
        ...
        {cardData.details_wordOrigin ? (
          <ScrollArea className='h-36 sm:h-48 w-full p-1 border rounded-md shadow-inner bg-background/30 dark:bg-background/40'>
            <div className='p-2 sm:p-3 w-full text-sm' lang='zh-CN'>
              <h4 className='font-semibold text-accent mb-1.5 text-md text-center'>
                词源信息:
              </h4>
              <p className='text-foreground/90 dark:text-foreground/80 leading-relaxed whitespace-pre-wrap text-left'>
                {cardData.details_wordOrigin}
              </p>
            </div>
          </ScrollArea>
      ...
      {cardData.exampleSentenceJapanese && (
        <div className='text-left'>
          <h4 className='font-semibold text-sm text-foreground/80 mb-0.5'>
            例:
          </h4>
          <p className='text-md font-medium text-foreground' lang='ja'>
            {cardData.exampleSentenceJapanese}
          </p>
          <p className='text-md font-medium text-foreground' lang='ja'>
            {cardData.exampleSentenceEnglish}
          </p>
        </div>
      )}
  ...
  return (
    <div
      ...
      aria-label={
        isFlipped
          ? `Showing details for ${cardData.japanese}`
          : `Showing Japanese: ${cardData.japanese}. Click to flip.`
      }>
      <Card
        className={`relative w-full h-full transform-style-preserve-3d transition-transform duration-700 ease-in-out shadow-xl group-hover:shadow-2xl ${
        ...
        <div
          className={`absolute inset-0 w-full h-full rounded-lg dark:bg-black/50 ${
            cardImageUri ? 'dark:bg-opacity-60' : 'dark:bg-opacity-0'
          }`}
        />

        <div className='absolute w-full h-full backface-hidden'>
          <div className='w-full h-full bg-card/80 dark:bg-transparent backdrop-blur-sm rounded-lg flex flex-col items-center justify-center p-1 relative'>
        ...
        <div className='absolute w-full h-full backface-hidden rotate-y-180'>
          <div className='w-full h-full bg-card/80 dark:bg-transparent backdrop-blur-sm rounded-lg flex flex-col items-start justify-between overflow-hidden p-2 sm:p-3 relative'>
            <BackContentInternal />
            {isGeneratingImage && !cardImageUri && (
              <div className='absolute inset-0 flex items-center justify-center bg-black/40 rounded-lg'>
                <Loader2 className='h-8 w-8 animate-spin text-white' />
